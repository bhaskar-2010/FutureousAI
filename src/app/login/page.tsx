...
import { LogIn, Mail, Eye, EyeOff, Phone, ArrowLeft, KeyRound } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getFriendlyErrorMessage(err) || "Failed to send OTP. Please check your phone number and try again.");
      if (window.recaptchaVerifier) {
        window.recaptchaVerifier.clear();
        window.recaptchaVerifier = null;
      }
        ...
        <div className="glass-strong rounded-2xl p-8 shadow-2xl">
          ...
          {authMethod === "default" ? (
            <>
              ...
              <div className="flex flex-col gap-3 mt-4">
                <button
                  ...
                  disabled={loading || isSigningIn || isSuccess}
                  className="w-full bg-white/5 border border-border text-foreground font-medium py-3 rounded-xl hover:bg-white/10 transition-colors flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {isSigningIn ? (
            ...
            <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300">
              ...
              >
                <ArrowLeft className="w-4 h-4" /> Back to Email/Password
              </button>

              {!showOtpInput ? (
                <form onSubmit={handleSendOtp} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium mb-1.5 text-muted-foreground" htmlFor="phone">
                      Phone Number
                    </label>
                    <input
                      id="phone"
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-primary to-primary-dark text-white font-semibold py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-60 shadow-lg shadow-primary/20 mt-2"
                  >
                    {loading ? "Sending..." : <><Phone className="w-4 h-4" /> Send OTP</>}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium mb-1.5 text-muted-foreground" htmlFor="otp">
                      Enter 6-digit OTP
                    </label>
                    <input
                      id="otp"
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-border bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all placeholder:text-muted-foreground/50 disabled:opacity-50 disabled:cursor-not-allowed tracking-[0.5em] text-center font-mono text-lg"
                      placeholder="------"
                      value={otp}
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-primary to-primary-dark text-white font-semibold py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-60 shadow-lg shadow-primary/20 mt-2"
                  >
                    {loading || isSuccess ? "Verifying..." : <><KeyRound className="w-4 h-4" /> Verify OTP</>}
                  </button>
                </form>
              )}
            </div>
